...
- X/Twitter 适配器读取当前激活的 For you/Following 时间线和单帖页面，分别使用 HomeTimeline/HomeLatestTimeline 与 TweetDetail GraphQL；时间线最多 5 页，单帖只请求一次目标帖子，不读取评论，正文使用 120,000 字符预算。X 的 ct0 不进入扩展存储。
