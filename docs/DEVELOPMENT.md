...
- Bilibili 字幕与当前视频元数据的保守 fallback。
- YouTube 字幕排序、时间戳和当前视频元数据的保守 fallback。
- 本地打包 PDF.js worker；PDF 文本给兼容端，原始 PDF 给 Kimi。

### 视频提取器优化 backlog（不阻断 V1）

Bilibili 和 YouTube 的字幕依赖登录态、站点接口及播放器页面结构，不能把“无字幕时的整页文本”当作可靠正文。当前实现只在拿到字幕时生成完整视频文稿；无字幕时仅保留标题/简介/播放器区域并显示 warning。

后续可独立优化：

- Bilibili：稳定 WBI 签名、分 P/互动视频 CID、字幕轨道权限和接口限流重试；AI 字幕质量筛选或音频转写。
