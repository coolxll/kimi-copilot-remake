...
- 知乎问题/回答提取器读取完整回答与受控评论树：回答页最多 100×5，问题页最多 20 个回答、每个回答 20×5，并共享 120,000 字符内容预算；评论分页校验回答归属，达到正常配额或字符预算静默停止，首个核心接口失败退回 DOM，后续接口失败保留部分内容并标记 warning。
- X/Twitter 适配器读取当前激活的 For you/Following 时间线和单帖页面，分别使用 HomeTimeline/HomeLatestTimeline 与 TweetDetail GraphQL；时间线最多 5 页、每帖评论最多 2 页/10 条，单帖评论最多 5 页/100 条，共享 120,000 字符预算，并过滤均衡规则识别的 spam。X 的 ct0 不进入扩展存储。
