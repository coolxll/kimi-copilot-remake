...
| ID | 场景 | Kimi Web | OpenAI Compatible | 验证方式 |
|---|---|---:|---:|---|
...
| P0-09 | Token 401 刷新一次、并发 single-flight | ✅ | 不适用 | 契约 fixture |
| P0-10 | API Root、Token 和 origin 权限 | 不适用 | ✅ | 权限手工 |
| P0-11 | SSE 任意字节边界与普通 JSON fallback | 不适用 | ✅ | unit |
| P0-12 | 429/5xx/网络重试与取消 | 不适用 | ✅ | unit/手工 |
| P0-13 | 长文分块、递归归并、截断 warning | 不适用 | ✅ | chunk unit + 手工 |
| P0-14 | Kimi 继续对话链接 | ✅ | 不适用 | 手工 |
| P1-01 | Bilibili 字幕/保守元数据 fallback | 🧪 | 🧪 | 手工；字幕接口优化待后续 |
| P1-02 | YouTube 字幕/保守元数据 fallback | 🧪 | 🧪 | 手工；无字幕转写待后续 |
| P1-03 | 文本型 PDF | ✅ | ✅ | 手工 |
